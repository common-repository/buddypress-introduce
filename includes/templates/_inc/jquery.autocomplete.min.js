 ...
 *   http://www.gnu.org/licenses/gpl.html
 */
jQuery.fn.autoCompletefb = function (b) {
    var c = this;
    var d = {
        ul: c, urlLookup: [""],
        acOptions: {},
    ...
    var a = { params: d, removeFind: function (e) {
        a.removeUsername(e); jQuery(e).unbind("click").parent().remove(); jQuery(d.inputClass, c).focus();
        return c.acfb
    },
        removeUsername: function (f) {
            var e = f.parentNode.id.split("-");
            jQuery("#send-to-usernames").removeClass(e[1])
        }
    };

    jQuery(d.foundClass + " img.p").click(function () { a.removeFind(this) });
    jQuery(d.inputClass, c).autocomplete(d.urlLookup, d.acOptions); jQuery(d.inputClass, c).result(function (k, l, j) {

        var j = d.foundClass.replace(/\./, "");
        var l = String(l).split(" (");
        var i = l[1].substr(0, l[1].length - 1);

        if (jQuery('#un-' + i).length || jQuery('.' + j).length > 10 ) 
            return jQuery(d.inputClass, c).val("");

        var h = '<li class="' + j + '" id="un-' + i + '"><span>' + l[0] + '</span> <span class="p">X</span></li>';
        var g = jQuery(d.inputClass, c).before(h); jQuery("#send-to-usernames").addClass(i); jQuery(".p", g[0].previousSibling).click(function () { a.removeFind(this) });
        jQuery(d.inputClass, c).val("");
    });
    jQuery(d.inputClass, c).focus();
    return a
};
